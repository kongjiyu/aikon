...
export default function VoucherCard({ title, subtitle, description, isActive = false }: VoucherCardProps) {
    ...
    return (
        <div className="h-full min-h-[250px] p-6 rounded-[10px] outline outline-[3px] outline-offset-[-3px] outline-gray-200 bg-white hover:outline-brand-dark transition-all flex flex-col justify-between gap-4 group cursor-pointer">
            <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
                    <h3 className="text-3xl font-bold text-brand-dark font-sans leading-tight group-hover:text-brand-dark transition-colors">
                        {title}
                    </h3>
                </div>
                <p className="text-sm font-semibold uppercase tracking-tight text-gray-500 group-hover:text-brand-dark">
                    {subtitle}
                </p>
            </div>
